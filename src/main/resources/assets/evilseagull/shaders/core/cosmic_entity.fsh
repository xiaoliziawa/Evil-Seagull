...
void main (void)
{
    // sample entity texture to create mask
    vec4 entityColor = texture(Sampler1, texCoord0);
    float maxC = max(entityColor.r, max(entityColor.g, entityColor.b));
    float minC = min(entityColor.r, min(entityColor.g, entityColor.b));
    float brightness = maxC;
    float saturation = maxC > 0.0 ? (maxC - minC) / maxC : 0.0;
    // cosmic on anything bright enough, but exclude saturated colors (yellow beak/feet)
    // and very dark pixels (black eyes, wing tips)
    float mask = smoothstep(0.12, 0.25, brightness) * (1.0 - smoothstep(0.35, 0.55, saturation));
