...
一个让 Alex's Mobs 的海鸥变得更加"聪明"的模组！现在它们不仅会从玩家手里偷食物，还学会了从你的精妙背包、AE2 ME 接口和 RS 接口里顺走食物。

## 功能特性

是的没错,byd海鸥会从你的精妙背包、ME接口和RS接口里面偷走你的食物了😈
...
- **从 RS 接口偷窃**：海鸥可以从精致存储（Refined Storage）的接口中偷取食物
...
- `rsInterfaceSearchRange`：RS 接口搜索范围（默认：8，范围：1-32）
...
2. 安装 Alex's Mobs 模组
3. 将本模组放入 `mods` 文件夹
4. （可选）安装 Sophisticated Backpacks、Applied Energistics 2 和/或 Refined Storage
5. 启动游戏
...
2. **世界方块检测**：海鸥会在配置范围内搜索放置的背包方块、ME 接口和 RS 接口
